...
// Emoji 
app.patch("/channels/:channelId/messages/:messageId/emoji/:emojiType", async (req, res) => {
  const {messageId} = req.params;
  const {userId} = req.session;
  const {emojiType} = req.params;
  const message = await Message.getById(messageId);
  let idx = -1;
  if (message != null) {
      switch (emojiType) {
          case "thumb":
              idx = message.thumb.indexOf(userId);
              (idx !== -1) ? message.thumb.splice(idx, 1) : message.thumb.push(userId);
              break;

          case "heart":
              idx = message.heart.indexOf(userId);
              (idx !== -1) ? message.heart.splice(idx, 1) : message.heart.push(userId);
              break;

          case "face":
              idx = message.face.indexOf(userId);
              (idx !== -1) ? message.face.splice(idx, 1) : message.face.push(userId);
              break;

          default:
              console.log("Issue with switches in index.js");
      }
      try {
          await message.save();
          fetchAndEmitMessages(socketServer);
